...
{
    // TODO: handle newlines in bytes.
    std.mem.copy(u8, self.buf[self.cursor.idx..], bytes);
    self.cursor.idx += bytes.len;
    self.cursor.col += bytes.len;
    self.eof += bytes.len;
}
...
pub const RendererInterface = struct {
    ...
    draw_cursor_rect_fn: DrawCursorRectSignature,
